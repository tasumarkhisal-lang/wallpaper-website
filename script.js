...
function saveRecentSearch(query) {
  ...
  if (recentSearches.length > 5) recentSearches.pop(); // Max 5 items
...
function openWallpaperDetail(photo) {
  ...
  const fullImg = document.getElementById('detailFullImg');
  const downloadBtn = document.getElementById('detailDownloadBtn');
  const shareBtn = document.getElementById('detailShareBtn');

  if (!modal || !fullImg) {
    // Agar detail modal HTML mein majood na ho, toh fallback lightbox open karein
    openModal(photo.src.original);
    return;
  }

  const titleName = photo.alt ? photo.alt.replace(/[^a-zA-Z0-9]/g, "_") : `wallpaper_${photo.id}`;

  fullImg.src = photo.src.large2x || photo.src.original;
...
function renderCard(photo) {
  ...
  const titleName = photo.alt ? photo.alt.replace(/[^a-zA-Z0-9]/g, "_") : `wallpaper_${photo.id}`;
  const photoEscaped = JSON.stringify(photo).replace(/'/g, "&apos;");

  card.innerHTML = `
    <img src="${photo.src.large}" alt="${photo.alt || 'Wallpaper'}" loading="lazy" onclick='openWallpaperDetail(${photoEscaped})'>
    <div class="overlay">
      <span class="photographer"><i class="fa-regular fa-user"></i> ${photo.photographer}</span>
      <div class="action-btns">
        <button class="icon-btn" onclick="openShareModal('${photo.src.original}')" title="Share">
          <i class="fa-solid fa-share-nodes"></i>
        </button>
        <button id="fav-btn-${photo.id}" class="icon-btn ${isFav ? 'liked' : ''}" onclick='toggleFavorite(${photoEscaped})' title="Favorite">
          <i class="fa-solid fa-heart"></i>
        </button>
        <button class="icon-btn" onclick="downloadImage('${photo.src.original}', '${titleName}')" title="Download">
